(function () {
  ...
  var eventDetails = {
    ...
    location:
      "Victory Doxa Passover Festival Grounds, opposite Electoral Commission, Lubowa, Kigo Rd, Kampala",
    description:
      "Golden Jubilee wedding anniversary celebration for Molly and Paul M. Wasswa."
  };

  var guests = window.WEDDING_GUESTS || {};
  var params = new URLSearchParams(window.location.search);
  var guestCode = (params.get("guest") || "").trim().toLowerCase();
  var guestName = guests[guestCode] || "Guest";
  ...
  function whatsappLink(name) {
    var message =
      "Hello, this is " +
      name +
      ". I am confirming attendance for Molly and Paul M. Wasswa's Golden Jubilee celebration on 4 September 2026.";
  ...
  function googleCalendarLink() {
    ...
    link.searchParams.set(
      "dates",
      calendarStamp(eventDetails.start) + "/" + calendarStamp(eventDetails.end)
    );
  ...
  function downloadCalendarFile() {
    var ics = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//Molly Paul Golden Jubilee//Invitation//EN",
      "BEGIN:VEVENT",
      "UID:molly-paul-golden-jubilee-20260904@example.com",
      "DTSTAMP:" + calendarStamp(new Date()),
      "DTSTART:" + calendarStamp(eventDetails.start),
      "DTEND:" + calendarStamp(eventDetails.end),
  ...
  function updateCountdown() {
    var now = new Date();
    var remaining = eventDetails.start.getTime() - now.getTime();
    ...
    Object.keys(values).forEach(function (key) {
      ...
      if (node) {
        node.textContent = String(values[key]).padStart(key === "days" ? 1 : 2, "0");
      }
    });
  }
  ...
  var rsvp = whatsappLink(guestName);
